...
if [ $(uname -s) == 'Darwin' ]; then
    echo
    echo "Are you running on mac OS X?"
    echo "This script cannot support directly non-Linux distributions."
    echo
    echo "NOTE: git should be already available."
    echo
    echo "NOTE: You should install python3 from here: "
...
cat <<-EOF
    ##################################################################
    This script will try to install on your UNIX system
    the pre-requisites necessary to run the RAPyDo framework.
...
do_install() {
    ...
    case $architecture in
        ...
        aarch64|arm64|ppc64le|s390x)
            cat 1>&2 <<-EOF
            Error: This install script does not support $architecture.
EOF
            exit 1
            ;;
        # not supported
