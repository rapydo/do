...
def test_command(command, *asserts):
    print("*********************************************")
    print(command)
...
def test_all(capfd):
    ...
    out = test_command("rapydo create test --auth xyz")
    assert "Invalid authentication service: xyz" in out

    out = test_command("rapydo create test --auth sql")
    assert "Missing frontend framework, add --frontend option" in out

    out = test_command("rapydo create test --auth sql --frontend xyz")
    assert "Invalid frontend framework: xyz" in out

    out = test_command("rapydo create test --auth sql --frontend angular")
    ...
    out = test_command("rapydo create test --auth sql --frontend angular --current --force")
    assert "Project test successfully created" in out

    # In this case the command should create/modify nothing... to be tested!
    out = test_command("rapydo create test --auth sql --frontend angular --current")
    assert "Project test successfully created" in out

    out = test_command("rapydo init")
    assert "Project initialized" in out

    out = test_command("rapydo pull")
    assert "Base images pulled from docker hub" in out

    # Skipping main because we are on a fake git repository
    out = test_command("rapydo update -i main")
    assert "All updated" in out

    # Selected a very fast service to speed up tests
    out = test_command("rapydo -s rabbit build --core")
    assert "Core images built" in out
    assert "No custom images to build" in out

    # Skipping main because we are on a fake git repository
    out = test_command("rapydo check -i main")
    assert "Checks completed" in out

    out = test_command("rapydo check -i main --check-permissions")
    assert "Checks completed" in out

    out = test_command("rapydo list --env")
    assert "List env variables:" in out

    out = test_command("rapydo list --args")
    assert "List of configured rapydo arguments:" in out

    out = test_command("rapydo list --active-services")
    assert "List of active services:" in out

    out = test_command("rapydo list --submodules")
    assert "List of submodules:" in out

    out = test_command("rapydo dump")
    ...
    out = test_command("rapydo dhparam")
    assert "No container found for proxy_1" in out

    out = test_command("rapydo formatter")
    ...
    out = test_command("rapydo version")
    # assert __version__ in out

    out = test_command("rapydo volatile backend --command hostname")
    # assert "backend-server" in out

    out = test_command("rapydo install --editable auto")
    out = test_command("rapydo install --user auto")
    out = test_command("rapydo install auto")
